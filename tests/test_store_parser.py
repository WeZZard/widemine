...
def test_session_title_prefers_ai_title_over_local_command_caveat(claude_projects: Path):
    ...
    bash_input = user_event(
        "bash", "meta", session, "<bash-input>zellij action new-pane</bash-input>"
    )
    ...
    write_jsonl(
        ...
        [
            ...
            {
                "type": "ai-title",
                "aiTitle": "Learn about collect workflow",
                "sessionId": session,
            },
...
def test_loads_main_and_nested_subagents(populated_projects: Path):
    ...
    assert child.agent_description == "Run Task"
...
def test_workflow_subagents_get_sibling_navigation(claude_projects: Path):
    ...
    workflow_children = [
        ...
        if child.agent_type == "workflow-subagent"
    ]
...
def test_duplicate_session_ids_are_unique_across_projects(claude_projects: Path):
    ...
    write_jsonl(
        second / f"{session}.jsonl", [user_event("u2", None, session, "Second project")]
    )
...
def test_system_events_keep_subtype_state_for_two_level_display(claude_projects: Path):
    ...
    system_events = [
        ...
        {"subtype": "informational", "content": "Bridge connected"},
        {
            ...
            "content": '{"message":"Bridge ready","url":"http://127.0.0.1:1"}',
        },
